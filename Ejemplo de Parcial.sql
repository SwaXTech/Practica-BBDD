...
CREATE PROCEDURE actualiza_precios @fecha DATE AS
BEGIN

	DECLARE @fecha_alta DATE, @manu_code CHAR(3), @stock_num INT, @descr VARCHAR(15), @unit_price DECIMAL(6,2);

	DECLARE novedades CURSOR FOR SELECT fecha_alta, manu_code, descr, unit_price FROM ##novedades;
	OPEN novedades;
			...
			BEGIN TRANSACTION
				...
				IF NOT EXISTS (SELECT stock_num FROM product_types WHERE stock_num = @stock_num)
					INSERT INTO product_types (stock_num, description)
						VALUES (@stock_num, @descr)

				IF NOT EXISTS (SELECT stock_num FROM products WHERE stock_num = @stock_num)
...
BEGIN
	...
	DECLARE llamadas CURSOR FOR SELECT i.call_dtime, i.call_code, d.call_code FROM inserted i LEFT JOIN deleted d ON i.customer_num = d.customer_num AND i.call_dtime = d.call_dtime;
	OPEN llamadas;
	...
	BEGIN
		...
		IF @last_call_code = NULL
			IF NOT EXISTS (SELECT 1 FROM resumen_llamadas WHERE anio = YEAR(@date) AND mes = MONTH(@date))
				INSERT INTO resumen_llamadas (anio, mes, call_code, cantidad)
					VALUES (YEAR(@date), MONTH(@date), @call_code, 1)
			ELSE
