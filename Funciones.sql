/*
Escribir una sentencia SELECT que devuelva el n�mero de orden, fecha de orden y el nombre del
d�a de la semana de la orden de todas las �rdenes que no han sido pagadas.


Si el cliente pertenece al estado de California el d�a de la semana debe devolverse en ingl�s, caso
contrario en espa�ol. 

Cree una funci�n para resolver este tema.
Nota: SET @DIA = datepart(weekday,@fecha)
Devuelve en la variable @DIA el nro. de d�a de la semana , comenzando con 1 Domingo hasta 7
S�bado.
*/
...
SELECT order_num, order_date, dbo.dia_de_la_semana(state, order_date)
FROM orders JOIN customer c ON orders.customer_num = c.customer_num
WHERE paid_date IS NULL 
GO

/*Escribir una sentencia SELECT para los clientes que han tenido �rdenes en al menos 2 meses
diferentes, los dos meses con las �rdenes con el mayor ship_charge.
Se debe devolver una fila por cada cliente que cumpla esa condici�n.*/
...
BEGIN
    ...
    BEGIN
		SELECT TOP 1
			@MES = MONTH(ship_date), @ANIO = YEAR(ship_date), @TOTAL = ship_charge
		FROM orders
    ...
    BEGIN
		SELECT TOP 1
			@MES = mes, @ANIO = anio, @TOTAL = ship_charge
		FROM (SELECT TOP 2
				MONTH(order_date) mes, YEAR(order_date) anio, ship_charge
			FROM orders
			WHERE customer_num = @CUSTOMER_NUM
...
SELECT DISTINCT stock_num, dbo.fabricantes_por_producto(stock_num)
FROM products
WHERE stock_num IN (SELECT stock_num
FROM catalog)
